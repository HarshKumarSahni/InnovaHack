...
import { Book, Upload, Trash2, Loader2, AlertCircle, FileText } from 'lucide-react';
...
const SyllabusPage: React.FC<SyllabusPageProps> = ({ API_BASE_URL, onBack, onSyllabusUploaded }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={onBack} className="text-sm font-medium text-cyan-600 hover:text-cyan-800">
      ...
      <form onSubmit={handleUpload} className="p-4 border-2 border-dashed border-gray-200 rounded-lg mb-6 space-y-4">
        ...
        <div>
          ...
          <input id="file-upload" type="file" onChange={handleFileChange} accept=".xlsx, .xls" className="file-input" required />
          <p className="text-xs text-gray-500 mt-1">Format: All topics must be in the first column, one per row.</p>
        </div>
        <button
          type="submit"
